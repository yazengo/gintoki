...
static void gc(uv_loop_t *loop, void *_p) {
	pipe_t *p = (pipe_t *)_p;
	strsink_t *ss = (strsink_t *)p->read.data;
...
static void grep_parse(strsink_t *ss, void *buf, int len) {
	int i = 0;
	for (;;) {
...
static void read_eof(strsink_t *ss) {
	pipe_t *p = ss->p;
...
// strsink() -- all
// strsink('grep', 'Duration:') -- grep the line has 'Duration:'
static int luv_strsink(lua_State *L, uv_loop_t *loop) {
	pipe_t *p = (pipe_t *)luv_newctx(L, loop, sizeof(pipe_t));
	strsink_t *ss = (strsink_t *)zalloc(sizeof(strsink_t));
...
void luv_strsink_init(lua_State *L, uv_loop_t *loop) {
	luv_register(L, loop, "strsink", luv_strsink);
}
